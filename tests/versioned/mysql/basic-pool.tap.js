...
tap.test('See if mysql is running', (t) => {
  setup(require('mysql'), (err) => {
    t.error(err, 'should not fail to set up mysql database')
    t.end()
  })
})

tap.test('bad config', (t) => {
  t.autoend()
...
tap.test('mysql built-in connection pools', {timeout: 30 * 1000}, (t) => {
  var helper = null
  var mysql = null
  var pool = null
...
tap.test('poolCluster', {timeout: 30 * 1000}, (t) => {
  t.autoend()
